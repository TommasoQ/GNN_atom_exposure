...
def create_model(config: dict) -> nn.Module:
    ...
    Args:
        config (dict): Model configuration. Key 'model_type' selects architecture:
            - 'gnn' (default): AtomExposureGNN (GATv2/GCN/GAT/GIN/GINE, 93 features)
            - 'minimal': MinimalGCN (lightweight GCN, 5 features, ~16K params)
            - 'simple': SimpleGCN (basic GCN baseline)
    ...
    if model_type == 'gnn':
        ...
        raise ValueError(f"Unknown model_type: {model_type}. Choose from: 'gnn', 'minimal', 'simple'")
